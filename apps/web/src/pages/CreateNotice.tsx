...
export default function CreateNotice() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const payload: Record<string, any> = { title, body, category, visibility };
      ...
      const serverErr = (err as any)?.response?.data;
      ...
      else if (serverErr?.error) setMsg(typeof serverErr.error === "string" ? serverErr.error : JSON.stringify(serverErr.error));
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <form onSubmit={onSubmit} className="space-y-4 bg-white rounded-2xl border shadow-sm p-6">
        ...
        <button disabled={loading} className="px-4 py-2 rounded bg-sky-600 text-white">{loading ? "Saving…" : "Save"}</button>
      </form>
    </div>
  );
}
