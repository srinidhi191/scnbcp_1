...
export const createNotice = async (req: any, res: any) => {
  try {
    ...
    const sanitizedAudience: any[] = Array.isArray(audience)
      ? audience
          .map((a: any) => {
            const out: any = { noticeId: undefined };
            if (a.role) out.role = a.role;
            if (a.departmentId) out.departmentId = a.departmentId;
            if (a.programId) out.programId = a.programId;
            if (a.batchId) out.batchId = a.batchId;
    ...
    if (visibility === "targeted" && sanitizedAudience.length) {
      ...
      const rows = sanitizedAudience.map((a: any) => {
        const row: any = { noticeId: notice._id };
        if (a.role) row.role = a.role;
        if (a.departmentId) row.departmentId = a.departmentId;
        if (a.programId) row.programId = a.programId;
        if (a.batchId) row.batchId = a.batchId;
        if (a.userId) row.userId = a.userId;
        return row;
      });
      await NoticeAudience.insertMany(rows);
    }
...
export const getAudience = async (req: any, res: any) => {
  try {
    const rows = await NoticeAudience.find({ noticeId: req.params.id });
...
export const listAcks = async (req: any, res: any) => {
  try {
    const rows = await NoticeAck.find({ userId: req.user._id });
    res.json({ acks: rows });
  } catch (err) {
